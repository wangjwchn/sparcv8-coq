...
Lemma signed_eq_zero :
  ...
  Proof.
    intros.
    assert (repr (signed i) = repr 0 ).
    subst;auto.
    rewrite H.
    auto.
    rewrite repr_signed in H0.
    auto.
  Qed.
...
Lemma unsigned_eq_zero :
  ...
  Proof.
    intros.
    assert (repr (unsigned i) = repr 0 ).
    subst;auto.
    rewrite H.
    auto.
    rewrite repr_unsigned in H0.
    auto.
  Qed.
...
Proof.
  ...
  remember (proj_sumbool_true _ H1).
  remember (proj_sumbool_true _ H4).
